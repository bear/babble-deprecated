...
class SingleServerIRCBot(SimpleIRCClient):
    ...
    def _on_namreply(self, c, e):
        ...
        for nick in string.split(e.arguments()[2]):
            foo = nick[1:]
            #print nick, foo, nick[0], self.channels[ch].is_oper(foo), self.channels[ch].is_hoper(foo)
            if nick[0] == "@":
                nick = nick[1:]
                self.channels[ch].set_mode("o", nick)
                self.channels[ch].clear_mode("h", nick)
                self.channels[ch].clear_mode("v", nick)
            elif nick[0] == "%":
                nick = nick[1:]
                self.channels[ch].set_mode("h", nick)
                self.channels[ch].clear_mode("o", nick)
                self.channels[ch].clear_mode("v", nick)
            elif nick[0] == "+":
                nick = nick[1:]
...
class Channel:
    ...
    def __init__(self):
        ...
        self.hoperdict = IRCDict()
        self.voiceddict = IRCDict()
        self.modes = {}

    def clear(self):
        for d in self.userdict, self.operdict, self.hoperdict, self.voiceddict:
            d.clear()
    ...
    def change_nick(self, before, after):
        ...
        if self.hoperdict.has_key(before):
            self.hoperdict[after] = 1
            del self.hoperdict[before]
